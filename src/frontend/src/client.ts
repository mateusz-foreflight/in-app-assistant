...
// RESOURCES
    ...
    export const getResourceById = (resourceId: number) =>
        fetch(baseUrl + `/resources/${resourceId}`).then(checkStatus);
...
// SOURCES
    ...
    export const getSourceById = (sourceId: number) =>
        fetch(baseUrl + `/sources/${sourceId}`).then(checkStatus);
